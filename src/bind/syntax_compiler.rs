...
pub struct SyntaxCompiler {
    ...
    substitute_frame_refs: Box<dyn Fn(&dyn Fn(usize) -> Option<CellRef>) -> SyntaxCompiler+Send+Sync>,
...
impl SyntaxCompiler {
    ...
    where ActionFn: 'static+Fn(CellRef) -> Result<CompiledActions, BindError>+Send+Sync {
        ...
        let substitute_frame_refs       = move |get_new_frame_ref: &dyn Fn(usize) -> Option<CellRef>| {
    ...
    where FrameRefFn: Fn(usize) -> Option<CellRef> {
