...
pub struct SyntaxCompiler {
    ...
    substitute_frame_refs: Box<dyn Fn(Box<dyn Fn(usize) -> usize>) -> SyntaxCompiler+Send+Sync>,
...
impl SyntaxCompiler {
    ...
    where ActionFn: 'static+Fn(CellRef) -> Result<CompiledActions, BindError>+Send+Sync {
        // Create a copy of the action generation function
        let generate_actions            = Arc::new(generate_actions);
        ...
        let substitute_frame_refs = move |get_new_frame_ref: Box<dyn Fn(usize) -> usize>| {
            ...
            SyntaxCompiler::with_compiler_and_reftype(move |param| generate_actions(param), new_parameter, reference_type)
        };
    ...
    ///
    pub fn substitute_frame_refs<FrameRefFn>(&self, get_new_frame_ref: FrameRefFn) -> SyntaxCompiler 
    where FrameRefFn: 'static+Fn(usize) -> usize {
        (self.substitute_frame_refs)(Box::new(get_new_frame_ref))
    }
}
...
///
fn substitute_frame_refs(original: CellRef, get_new_frame_ref: &dyn Fn(usize) -> usize) -> CellRef {
    use self::SafasCell::*;

    match &*original {
        FrameReference(cell, 0, ref_type)   => FrameReference(get_new_frame_ref(*cell), 0, *ref_type).into(),
