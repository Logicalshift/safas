...
pub struct SyntaxCompiler {
    ...
    substitute_frame_refs: Box<dyn Fn(&dyn Fn(FrameReference) -> Option<CellRef>) -> SyntaxCompiler+Send+Sync>,
...
impl SyntaxCompiler {
    ...
    where ActionFn: 'static+Fn(CellRef) -> Result<CompiledActions, BindError>+Send+Sync {
        ...
        let substitute_frame_refs       = move |get_new_frame_ref: &dyn Fn(FrameReference) -> Option<CellRef>| {
    ...
    where FrameRefFn: Fn(FrameReference) -> Option<CellRef> {
        (self.substitute_frame_refs)(&get_new_frame_ref)
    }
}
...
///
fn substitute_frame_refs(original: CellRef, get_new_frame_ref: &dyn Fn(FrameReference) -> Option<CellRef>) -> CellRef {
    match &*original {
