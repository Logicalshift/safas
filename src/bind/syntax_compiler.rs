...
pub struct SyntaxCompiler {
    ...
    substitute_frame_refs: Box<dyn Fn(Box<dyn Fn(usize) -> Option<CellRef>>) -> SyntaxCompiler+Send+Sync>,
...
impl SyntaxCompiler {
    ...
    where ActionFn: 'static+Fn(CellRef) -> Result<CompiledActions, BindError>+Send+Sync {
        ...
        let substitute_frame_refs = move |get_new_frame_ref: Box<dyn Fn(usize) -> Option<CellRef>>| {
    ...
    where FrameRefFn: 'static+Fn(usize) -> Option<CellRef> {
...
///
fn substitute_frame_refs(original: CellRef, get_new_frame_ref: &dyn Fn(usize) -> Option<CellRef>) -> CellRef {
    use self::SafasCell::*;
